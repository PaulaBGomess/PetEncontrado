...
export function AuthProvider({children}:{children:React.ReactNode}){
  ...
  const register=useCallback(async(d:any)=>{
    const r=await apiFetch('/auth/register',{method:'POST',body:JSON.stringify(d)});
    setToken(r.accessToken);
    setUser(r.user);
  },[]);
  ...
  const v=useMemo(()=>({user,token,loading,login,register,logout,refresh}),[user,token,loading,login,register,logout,refresh]);
